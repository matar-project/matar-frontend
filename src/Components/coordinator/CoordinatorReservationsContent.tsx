...
import { useRejectVolunteerReservationMutation } from '../../Hooks/coordinator/mutations/useRejectVolunteerReservationMutation';
import { useCoordinatorBooksQuery } from '../../Hooks/coordinator/queries/useCoordinatorBooksQuery';
import { useRequestReservationsQuery } from '../../Hooks/coordinator/queries/useRequestReservationsQuery';
import { COORDINATOR_REQUEST_TYPE_LABELS } from '../../constants/coordinator.constants';
...
function BookCard({ request }: { request: WorkflowRequest }) {
  ...
  const { data: reservations, isLoading: loadingReservations } =
    useRequestReservationsQuery(request.id, expanded);
  ...
  const rejectReservation =
    useRejectVolunteerReservationMutation(request.id);
...
export function CoordinatorReservationsContent() {
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const debouncedSearch = useDebouncedValue(search, 500);

  const { data, isLoading } = useCoordinatorBooksQuery(
    page,
    debouncedSearch,
  );
